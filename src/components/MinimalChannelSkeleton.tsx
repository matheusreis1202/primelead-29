...
interface MinimalChannelSkeletonProps {
  viewMode?: 'grid' | 'list';
  index?: number;
}

export const MinimalChannelSkeleton = React.memo(({ 
  viewMode = 'grid', 
  index = 0 
}: MinimalChannelSkeletonProps) => {
  if (viewMode === 'list') {
    return (
      <Card 
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              <Skeleton className="absolute -top-1 -right-1 w-6 h-4 rounded-full bg-[#555]" />
            </div>

            {/* Info */}
            <div className="flex-1 space-y-2">
              <Skeleton className="h-5 w-3/4 bg-[#333]" />
              <Skeleton className="h-4 w-24 bg-[#555] rounded-full" />
            ...
            <div className="grid grid-cols-3 gap-4 w-72">
              {[1, 2, 3].map(i => (
                <div key={i} className="text-center space-y-1">
                  <Skeleton className="h-4 w-4 rounded bg-[#555] mx-auto" />
            ...
            <div className="flex gap-2">
              <Skeleton className="h-9 w-20 bg-[#555] rounded" />
  ...
  // Grid view - apenas preto e cinza
  return (
    <Card 
      ...
      <CardContent className="p-4 h-full flex flex-col">
        ...
        <div className="flex justify-center mb-3 mt-2">
          <div className="relative">
            ...
            <Skeleton className="absolute -top-1 -right-1 w-5 h-4 rounded-full bg-[#555]" />
          </div>
        </div>
        ...
        <div className="text-center mb-3 space-y-2">
          ...
          <Skeleton className="h-3 w-20 bg-[#555] rounded-full mx-auto" />
        ...
        <div className="space-y-2 mb-4 flex-1">
          {[1, 2, 3].map(i => (
            <div key={i} className="flex items-center gap-2 p-2 bg-[#0D0D0D] rounded-lg">
              <Skeleton className="h-3 w-3 rounded bg-[#555]" />
        ...
        <div className="space-y-2 mt-auto">
          <Skeleton className="h-8 w-full bg-[#555] rounded" />
...
MinimalChannelSkeleton.displayName = 'MinimalChannelSkeleton';
